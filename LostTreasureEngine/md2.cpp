...
//precalculated normal vectors of md2 models
vec3_t	MD2Model::anorms[NUMVERTEXNORMALS] = {
...
float	MD2Model::anorms_dots[SHADEDOT_QUANT][256] = {
...
static float	*shadedots = MD2Model::anorms_dots[0];
...
bool MD2Model::LoadModel(const char *filename)
{
 ...
 unsigned int MD2Model::LoadTexture(const char* filename)
{
	 //loadbmp into bmploader object
	 BMPLoader b(filename);

	//gen texture
	glGenTextures(1, &gl_texture_object);
	//bind it
	glBindTexture(GL_TEXTURE_2D, gl_texture_object);
	//setup texture pixel unpack
...
//calculate the current frame, next frame and interpolation percent
...
// process all lighting calcs.
void MD2Model::ProcessLighting(void)
{
...
//interpolate and scale vertices from the current and the next frame
void MD2Model::Interpolate(vec3_t *vertlist)
{
	...
	//create current frame and next frame's vertex list from the whole vertex list
...
//draw the current model frame using OpenGL commands
...
//draw one frame of the model using gl commands
...
//initialize m_anim from the specified animation
